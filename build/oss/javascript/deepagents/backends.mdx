...
| [Context Hub](#contexthubbackend) | `agent = create_deep_agent(model="google_genai:gemini-3.1-pro-preview", backend=ContextHubBackend("my-agent"))` <br></br>**Python SDK only.** Stores files durably in a LangSmith Hub repo, without provisioning a separate LangGraph store. |
...
`ContextHubBackend` stores files in a LangSmith Hub repo. Construct it with a repo identifier in `owner/name`, `-/name`, or `name` format.
...
**Best for:**
- LangSmith-native durable filesystem persistence in Python without separately wiring a LangGraph `BaseStore`.
