...
This page covers:
- **[Deployment options](#deployment-options)**: choose Managed Deep Agents or standard LangSmith Deployment
...
Deep Agents support two production deployment paths:

| Path | Use when |
| --- | --- |
| [Managed Deep Agents](/oss/javascript/deepagents/deploy) | You want the fastest managed path for deploying a Deep Agents project with the `deepagents deploy` CLI and the Managed Deep Agents API. |
| [LangSmith Deployment](/langsmith/deployment-quickstart) | You need custom application code, custom routes, advanced authentication, full Agent Server APIs, stronger isolation controls, or maximum scalability. |

Managed Deep Agents is in private preview and exposes managed agent and thread APIs under `/v1/deepagents`.

For the CLI-based Managed Deep Agents path, see [Deploy with Managed Deep Agents](/oss/javascript/deepagents/deploy). For the full LangSmith guide, see [Deploy a Managed Deep Agent](/langsmith/deploy-managed-deep-agent). For standard LangSmith Deployment setup, see the [LangSmith Deployments quickstart](/langsmith/deployment-quickstart).

The remaining code examples on this page use LangSmith Deployment directly.
