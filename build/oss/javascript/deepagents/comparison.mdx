...
Managed Deep Agents provides a hosted API for managed agents, threads, and runs. For self-hosted deployments, LangSmith [Agent Server](/langsmith/agent-server) provides streaming endpoints, thread management, run history, webhooks, and [authentication](/langsmith/auth).
...
Deep agents support managed and self-hosted deployment paths:

- **Managed:** create or update a [Managed Deep Agent](/oss/javascript/deepagents/deploy) with [`deepagents deploy`](/oss/javascript/deepagents/deploy).
