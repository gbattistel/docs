...
| API | Use for |
| --- | ------- |
| [Event Streaming](/oss/javascript/langgraph/streaming/event-streaming) | Application and frontend streaming via typed projections (`run.messages`, `run.values`, `run.subgraphs`, `run.output`). |
| [Pregel Streaming](/oss/javascript/langgraph/streaming) | Low-level access to Pregel stream modes (`updates`, `values`, `messages`, `custom`, `checkpoints`, `tasks`, `debug`). |
...
| Build a custom stream projection primitive | [StreamChannel](/oss/javascript/langgraph/streaming/streamchannel) |
