...
The typed projections (`run.messages`, `run.values`, etc.) are built from these channels. Filter on channels when [subscribing to raw protocol events](#subscribe-to-raw-protocol-events) from a deployed graph.
