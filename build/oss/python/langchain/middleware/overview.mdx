...
Middleware is not a separate runtime—it compiles down into the same [LangGraph](/oss/python/langgraph/overview) nodes and edges that power [`create_agent`](https://reference.langchain.com/python/langchain/agents/factory/create_agent). Because `create_agent` returns a compiled graph, you can drop the whole agent (middleware and all) into a larger `StateGraph` as a node or subgraph, and every middleware hook continues to run.
...
The HITL interrupt, summarization, PII redaction, retries, and any custom hooks all travel with the agent node. See [Use subgraphs](/oss/python/langgraph/use-subgraphs) for the full set of composition patterns, including per-invocation versus per-thread persistence.
