...
| [Context Hub (LangSmith Hub)](#contexthubbackend-langsmith-hub) | `agent = create_deep_agent(model="google_genai:gemini-3.1-pro-preview", backend=ContextHubBackend("-/my-agent"))` <br></br>**Python SDK only.** Stores files durably in a LangSmith Hub repo, without provisioning a separate LangGraph store. |
...
### ContextHubBackend (LangSmith Hub)
