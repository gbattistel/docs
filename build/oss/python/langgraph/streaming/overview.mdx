...
| API | Use for |
| --- | ------- |
| [Event Streaming](/oss/python/langgraph/streaming/event-streaming) | Application and frontend streaming via typed projections (`run.messages`, `run.values`, `run.subgraphs`, `run.output`). |
| [Pregel Streaming](/oss/python/langgraph/streaming) | Low-level access to Pregel stream modes (`updates`, `values`, `messages`, `custom`, `checkpoints`, `tasks`, `debug`). |
...
| Build a custom stream projection primitive | [StreamChannel](/oss/python/langgraph/streaming/streamchannel) |
