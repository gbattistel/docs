...
Each event has a channel-like `method`, a `seq` number, and `params` containing `namespace`, `timestamp`, optional `node`, and channel-specific `data`. To subscribe to specific graph, subgraph, or task namespaces, see [Namespaces](/oss/python/langgraph/streaming/namespaces).
...
Use `namespaces` and `depth` to subscribe to specific graph, subgraph, and task events. See [Namespaces](/oss/python/langgraph/streaming/namespaces) for matching rules and recipes.
...
Event Streaming and Pregel Streaming serve different levels of the stack. Use [Pregel Streaming](/oss/python/langgraph/streaming) when you need low-level Pregel stream modes or access to LangGraph internals. Use Event Streaming (`stream_events(..., version="v3")`) when application code benefits from typed projections.
